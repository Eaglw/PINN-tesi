...
def soluzione_analitica(x, y, Lx=1.0, Ly=1.0, Nx=50):
    ...
    const_pi = torch.tensor(np.pi, device=x.device, dtype=x.dtype)
...
torch.set_default_dtype(torch.float64) # Carichiamo tutto in 64, convertiremo nel loop

goal = [0, 1, 2, 3] # I 4 casi richiesti
...
base_output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'experiments_hybrid_40k')
results_csv_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'results.csv')

# --- GENERAZIONE DATI MASTER (In FP64) ---
Lx, Ly = 1.0, 1.0
Nx_dom, Ny_dom = 50, 50
...
xy_master_grid = generate_grid_points(40, 40, Lx, Ly, margin=1e-4, device=device)
xy_master_random = generate_internal_points(1600, Lx, Ly, margin=1e-4, device=device)
...
# Pre-calcolo soluzioni analitiche
...
print(f"Starting HYBRID Grid Search (Adam@32 -> L-BFGS@64)")

for layers_config in layers_options:
    for act_fn in activation_options:
        config_name = f"Hybrid_L{format_layers_name(layers_config)}_E{epochs}_{act_fn.__name__}"
        ...
        if 0 in goal:
            ...
            update_results_csv(results_csv_path, {'Timestamp': datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 'Architecture': str(layers_config), 'Activation_Func': act_fn.__name__, 'Run_Type': 'NN_Random_Hybrid', 'Time': time.time()-start, 'L2_Error': l2})
        ...
        if 1 in goal:
            ...
            l2, _ = compute_metrics(model, xy_grid_flat, T_grid)
            update_results_csv(results_csv_path, {'Timestamp': datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 'Architecture': str(layers_config), 'Activation_Func': act_fn.__name__, 'Run_Type': 'NN_Grid_Hybrid', 'Time': time.time()-start, 'L2_Error': l2})
        ...
        if 2 in goal:
            ...
            train_hybrid_logic(model, (xy_pinn_data, T_pinn_data), (xy_master_boundary, T_master_boundary), validation_grid_tuple, 
                               epochs=epochs, collocation_points=xy_master_grid, lr_strategy=lr_strategy, final_dir=config_dir,
                               loss_weights={'data': 1.0, 'bc': 1.0, 'physics': 1.0}, case_name="2_PINN_DataPhys")
            l2, _ = compute_metrics(model, xy_grid_flat, T_grid)
            update_results_csv(results_csv_path, {'Timestamp': datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 'Architecture': str(layers_config), 'Activation_Func': act_fn.__name__, 'Run_Type': 'PINN_DataPhys_Hybrid', 'Time': time.time()-start, 'L2_Error': l2})
        ...
        if 3 in goal:
            ...
            l2, _ = compute_metrics(model, xy_grid_flat, T_grid)
            update_results_csv(results_csv_path, {'Timestamp': datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 'Architecture': str(layers_config), 'Activation_Func': act_fn.__name__, 'Run_Type': 'PINN_PurePhys_Hybrid', 'Time': time.time()-start, 'L2_Error': l2})

print("\nHybrid Grid Search completed.")
