...
# --- Parametri Fisici TARGET (Ground Truth per questo Step di Continuazione) ---
...
# Fattore di perturbazione per i parametri del problema inverso (0.80 = 80% del valore reale target)
GUESS_FACTOR = 0.80
...
# --- Iperparametri di Training Transfer Learning (Fase 1: Cinematica & Reologia) ---
ADAM_EPOCHS_PHASE1 = 20000
USE_LBFGS_PHASE1 = True
LBFGS_MAX_ITERS_PHASE1 = 5000

# Fase 2 disattivata per concentrarsi sulla Fase 1
...
W_CONSTITUTIVE = 1.0    # Peso equazione costitutiva reologica (Oldroyd-B)

W_DATA_2 = 20.0
W_BC_2 = 5.0
W_MOMENTUM = 0.0        # Momentum spento in Fase 1
W_DRIFT = 0.0
VARIANCE_EPS = 1e-4
...
strategy_tag = "TRANSFER_LEARNING"
...
if __name__ == "__main__":
    ...
    # 2b. Caricamento Pesi da Transfer Learning o Ripresa In-Flight
    if RESUME_INFLIGHT_PATH is not None and os.path.exists(RESUME_INFLIGHT_PATH):
        print(f"\n[Ripresa In-Flight] Trovato checkpoint di ripresa: {RESUME_INFLIGHT_PATH}")
        resume_ckpt_for_train = RESUME_INFLIGHT_PATH
    else:
        resume_ckpt_for_train = None
        if TRANSFER_SOURCE_CHECKPOINT.exists():
            print(f"\n[Transfer Learning] Caricamento pesi pre-addestrati da:\n  {TRANSFER_SOURCE_CHECKPOINT}")
            source_chk = torch.load(str(TRANSFER_SOURCE_CHECKPOINT), map_location=DEVICE)
            ...
            print(f"[Transfer Learning] Caricati con successo {len(pretrained_dict)} tensori di pesi per la rete.")
            ...
            print(f"\n[ATTENZIONE] Checkpoint sorgente NON TROVATO: {TRANSFER_SOURCE_CHECKPOINT}")
            print("Il training partirà da pesi casuali.")

    # Recap Configurazione
    total_params = sum(p.numel() for p in model.parameters())
    print(f"\nModello: {total_params:,} parametri totali")
    if INVERSE_PROBLEM:
        print("Modalità: PROBLEMA INVERSO - TRANSFER LEARNING FASE 1 (Cinematica & Reologia)")
        print(f"  - Obiettivo: Identificazione lambda={LAM_TRUE}s e mu_p={MU_P_TRUE}Pa·s")
    ...
    history = train(
        ...
        physics, 
        data, 
        resume_checkpoint=resume_ckpt_for_train,
        save_dir=OUTPUT_DIR,
        tb_writer=tb_writer
    )
    ...
    # Salvataggio del checkpoint permanente di continuazione
    permanent_ckpt_path = BASE_DIR / "checkpoints" / f"checkpoint_inverso_fase1_{PARAM_TAG}_transfer.pth"
    torch.save({
        ...
        'param_tag': PARAM_TAG,
        'target_lam': LAM_TRUE,
        'target_mu_p': MU_P_TRUE,
        'target_mu_s': MU_S_TRUE,
    }, permanent_ckpt_path)
    print(f"\n[Checkpoint Permanente] Salvato in: {permanent_ckpt_path}")
