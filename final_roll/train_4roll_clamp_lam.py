...
# ============================================================================
# 2. FISICA PERSONALIZZATA CON CLAMP SU LAMBDA E SOLO LAMBDA ADDESTRABILE
# ============================================================================
MIN_LAM_CLAMP = 0.035

class PhysicsClampedLam(Physics):
    """
    Variante della classe Physics in cui:
    1. Il valore del tempo di rilassamento lambda è vincolato ad essere >= MIN_LAM_CLAMP (es. 0.035).
    2. SOLO lambda è addestrabile; tutti gli altri parametri fisici (beta, eps, alpha, mu_s)
       sono fissati ai rispettivi valori reali (ground truth).
    """
    def __init__(self, *args, min_lam_clamp=MIN_LAM_CLAMP, train_only_lam=True, **kwargs):
        self.min_lam_clamp = min_lam_clamp
        self.train_only_lam = train_only_lam
        super().__init__(*args, **kwargs)

        if self.train_only_lam:
            # Forza tutti i parametri tranne lam ad essere NON addestrabili e fissati al valore reale
            beta_true = getattr(builtins, "BETA_TRUE", 0.10)
            eps_true = getattr(builtins, "EPS_TRUE", 0.0)
            alpha_true = getattr(builtins, "ALPHA_TRUE", 0.0)
            mu_s_true = getattr(builtins, "MU_S_TRUE", 0.1)

            with torch.no_grad():
                self._raw_beta.copy_(inverse_softplus(beta_true))
                self._raw_eps.copy_(inverse_softplus(eps_true))
                self._raw_alpha.copy_(inverse_softplus(alpha_true))
                self._raw_mu_s.copy_(inverse_softplus(mu_s_true))

            self._raw_beta.requires_grad_(False)
    ...
    def set_trainable(self, name, trainable=True):
        """Overridden per impedire che altri parametri vengano sbloccati se train_only_lam è True."""
        if self.train_only_lam and name != "lam":
...
config_name = f"clamp_lam_{MIN_LAM_CLAMP}_onlyLam_noStressBC_{DATASET_PATH.stem}_L{layers_str}_E{ADAM_EPOCHS_PHASE1+ADAM_EPOCHS_PHASE2}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print(f"ESPERIMENTO: NO STRESS BC + CLAMP LAMBDA >= {MIN_LAM_CLAMP} (SOLO LAMBDA ADDESTRABILE)")
    print(f"  - Stress BC rulli: {USE_ROLL_STRESS_BC}")
    print(f"  - Lambda initial guess: {GUESS_LAM} (target: {LAM_TRUE}, min clamp: {MIN_LAM_CLAMP})")
    print(f"  - Parametri fissati: beta={BETA_TRUE}, eps={EPS_TRUE}, alpha={ALPHA_TRUE}, mu_s={MU_S_TRUE}")
    print("=" * 60)

    # 1. Caricamento Dati
    data = load_data()

    # 2. Inizializzazione Modello e Fisica Clamped (Solo Lam Addestrabile)
    ...
    # Recap Configurazione Parametri Trainable
    total_params = sum(p.numel() for p in model.parameters())
    trainable_phys = [name for name, p in physics.named_parameters() if p.requires_grad]
    print(f"\nModello: {total_params:,} parametri totali delle reti neurali")
    print(f"Parametri fisici addestrabili: {trainable_phys}")
    print(f"  lam: {physics.lam.item():.6f} (target: {LAM_TRUE}, min clamp: {MIN_LAM_CLAMP})")
    print(f"  beta: {physics.beta.item():.6f} (fisso)")
    print(f"  eps: {physics.eps.item():.6f} (fisso)")
    print(f"  alpha: {physics.alpha.item():.6f} (fisso)")
