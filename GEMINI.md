...
- **Convenzione Nomenclatura Parametrica `L-P-S` (Tassativa)**: Per identificare univocamente le configurazioni fisiche dei fluidi viscoelastici (Oldroyd-B), utilizzare SEMPRE lo schema `L{lambda}-P{eta_p}-S{eta_s}`:
  - Dataset COMSOL: `COMSOL/4roll/4_roll_mill_L{lambda}-P{eta_p}-S{eta_s}.csv` (es. `4_roll_mill_L0.05-P0.5-S0.5.csv`).
  - Checkpoint Fase 1: `checkpoint_inverso_fase1_L{lambda}-P{eta_p}-S{eta_s}_40k+10k.pth`.
  - Cartelle di output run: devono contenere obbligatoriamente il tag `[{PARAM_TAG}]`, ad esempio `[{timestamp}][INV][STAGED][L0.05-P0.5-S0.5][{budget_tag}]`. Questa convenzione si applica rigorosamente sia a `train_4roll_main.py` sia a `train_4roll_main_mauri.py`.